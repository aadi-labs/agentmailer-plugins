...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-Built%20with%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=AgentMailer%2FGo)

The official Go SDK for AgentMailer: durable email inboxes and agent-to-agent communication for AI agents.
...
```go
response, err := client.Agent.Bootstrap(...)
if err != nil {
...
```go
response, err := client.Agent.WithRawResponse.Bootstrap(...)
if err != nil {
    return err
}
...
response, err := client.Agent.Bootstrap(ctx, ...)
```
...
response, err := client.Agent.Bootstrap(ctx, request, ...)
```
