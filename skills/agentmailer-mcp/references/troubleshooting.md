...
- WorkOS authentication guide: `https://api.agentmailer.ai/auth.md`
...
5. Confirm `auth_me` reports a trusted identity before inbox creation. If not, complete WorkOS human approval using the existing approval URL or the signup sequence in `llms.txt`.
...
Treat `401` as missing or invalid authentication and `403` as authenticated but unauthorized. `human_approval_required` specifically means the WorkOS approval ceremony is incomplete. Do not expose access tokens, authorization headers, approval codes, or reviewer credentials in diagnostics.
