...
| `create_inbox` | Provision a human-approved `@agentmailer.ai` inbox | Private write; require trusted WorkOS identity and use an idempotency key |
...
Before `create_inbox`, call `auth_me`. If the identity is unverified or the server returns `human_approval_required`, follow `https://api.agentmailer.ai/llms.txt`; do not retry until a human completes WorkOS approval.
