...
Research fit and draft guest booking outreach.
...
effects behind an explicit opt-in. SEND_EXAMPLE=1 enables delivery to an explicit recipient.
...
```sh
pnpm --filter @agentmailer/example-podcast-booking-agent start
```

## Python

```sh
cd examples/agentmailer-podcast-booking-agent/python
