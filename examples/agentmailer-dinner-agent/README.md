# Dinner coordination agent

Coordinate participants, cuisine preferences, constraints, and a final dinner plan.
...
effects behind an explicit opt-in. CREATE_DRAFT_EXAMPLE=1 creates a draft but never sends it.
...
```sh
pnpm --filter @agentmailer/example-dinner-agent start
```

## Python

```sh
cd examples/agentmailer-dinner-agent/python
